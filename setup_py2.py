...
PACKAGE_NAME = "django-rest-framework-security"
PACKAGE_DOWNLOAD_URL = (
    "https://github.com/Nekmo/django-rest-framework-security/archive/master.zip"
)
MODULE = "rest_framework_security"
REQUIREMENTS_FILES = [
    {"name": "common-requirements.txt"},
    {
        "name": "py2-requirements.txt",
        "marker": 'python_version<"3.0"',
        "include": sys.version_info < (3, 0),
    },
    {
        "name": "py3-requirements.txt",
        "marker": 'python_version>"3.0"',
        "include": sys.version_info > (3, 0),
    },
]
STATUS_LEVEL = (
    5  # 1:Planning 2:Pre-Alpha 3:Alpha 4:Beta 5:Production/Stable 6:Mature 7:Inactive
)
KEYWORDS = ["django-rest-framework-security"]
LICENSE = "MIT license"
...
def read_requirements_files(files):
    ...
    for file in files:
        if StrictVersion(setuptool_version) >= StrictVersion("20.2"):
            reqs.extend(
                [
                    ("{};{}".format(req, file["marker"]) if file.get("marker") else req)
...
def get_python_versions(string_range):
    if "-" not in string_range:
        return [string_range]
    return [
        "{0:.1f}".format(version * 0.1)
        for version in range(
            *[
                int(x * 10) + (1 * i)
                for i, x in enumerate(map(float, string_range.split("-")))
            ]
        )
    ]
...
def get_python_classifiers(versions):
    for version in range(2, 4):
        if not next(
            iter(filter(lambda x: int(float(x)) != version, versions.copy())), False
        ):
...
def get_platform_classifiers(platform):
    ...
    return [
        "Operating System :: {}".format(" :: ".join(parts[: i + 1]))
...
python_versions = set(
    chain(*[get_python_versions(versions) for versions in PYTHON_VERSIONS])
) - {2.8, 2.9}
status_name = [
    "Planning",
    "Pre-Alpha",
...
classifiers.extend(
    chain(*[get_platform_classifiers(platform) for platform in PLATFORMS])
)
classifiers.extend(
    [

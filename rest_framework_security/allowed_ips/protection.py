...
from rest_framework_security.allowed_ips.emails import (
    WarnNewIpEmail,
    WarnIpEmail,
    DenyNewIpEmail,
)
...
def get_default_ip_action(user):
    if config.ALLOWED_IPS_USER_IP_CONFIG_MODEL:
        settings_model_class: Type["UserIpSettingsBase"] = import_string(
            config.ALLOWED_IPS_USER_IP_CONFIG_MODEL
        )
        settings: Union[
            "UserIpSettingsBase", None
        ] = settings_model_class.objects.filter(user=user).first()
    else:
        settings = None
    return (
        settings.default_ip_action if settings else config.ALLOWED_IPS_DEFAULT_IP_ACTION
    )
...
class AllowedIpsProtection:
    ...
    def get_action(self) -> Tuple[str, Union[UserIp, None]]:
        ...
        """
        user_ip: Union[UserIp, None] = UserIp.objects.filter(
            user=self.user, ip_address=self.ip
        ).first()

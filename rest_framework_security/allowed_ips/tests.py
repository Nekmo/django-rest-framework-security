...
class AllowedIpsProtectionTestCase(TestCase):
    def setUp(self):
        ...
        self.user_ip: UserIp = UserIp.objects.create(
            ip_address=self.ip, user=self.user, action="allow"
        )
    ...
    @patch(
        "rest_framework_security.allowed_ips.protection.get_default_ip_action",
        return_value="deny",
    )
    ...
    def test_deny_new_ip(self, m1, m2):
        ...
        user_ip = UserIp.objects.filter(
            user=self.user, ip_address=ip, action=""
        ).first()

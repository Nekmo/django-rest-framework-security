...
from rest_framework_security.otp.exceptions import OTPStaticTokensAlreadyExistsException, OTPDevicesRequiredException
...
class OTPStaticQuerySet(models.QuerySet):
    def create_tokens(self, user):
        ...
        if self.filter(user=user).exists():
            raise OTPStaticTokensAlreadyExistsException(f'Static tokens have already been created for user {user}')
        if not OTPDevice.objects.filter(user=user).exists():
            raise OTPDevicesRequiredException('Create otp devices first to create static OTP tokens')

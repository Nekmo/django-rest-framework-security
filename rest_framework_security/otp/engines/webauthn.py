...
class WebAuthnEngine:
    ...
    def challenge(self, request, otp_device):
        ...
        webauthn_user = webauthn.WebAuthnUser(
            ukey,
            otp_device.user.username,
            otp_device.user.username,
            otp_device.data["icon_url"],
            otp_device.data["credential_id"],
            otp_device.data["pub_key"],
            otp_device.counter,
    ...
    def verify(self, request, otp_device, data):
        ...
        webauthn_assertion_response = webauthn.WebAuthnAssertionResponse(
            webauthn_user, data, challenge, self._get_origin(request), uv_required=False
        )  # User Verification
        try:

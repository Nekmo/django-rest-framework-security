...
class OTPDeviceViewSetTestCase(APITestCase):
    ...
    @patch('rest_framework_security.otp.engines.webauthn.webauthn.WebAuthnRegistrationResponse')
    def test_webauthn_create(self, m):
        ...
        self.client.force_authenticate(self.user)
        response = self.client.post(reverse('otpdevice-begin-register'),
                                    req_data, HTTP_HOST='127.0.0.1', format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.post(reverse('otpdevice-list'), dict(req_data, data={
            'pub_key': '',
            'credential_id': '',
            'rp_id': '',
            'icon_url': ''
        }), HTTP_HOST='127.0.0.1', format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(OTPDevice.objects.count(), 1)
    ...
    def test_webauthn_verify(self, m):
        ...
        otp_device = OTPDevice.objects.create(
            otp_type='webauthn', destination_type='device', data={
                'pub_key': '',
                'credential_id': '',
                'rp_id': '',
                'icon_url': ''
        ...
        self.client.force_authenticate(self.user)
        self.client.get(reverse('otpdevice-challenge', kwargs={'pk': otp_device.pk}),
                        {}, format='json')
        response = self.client.post(reverse('otpdevice-verify', kwargs={'pk': otp_device.pk}), {}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(self.client.session['otp'], False)

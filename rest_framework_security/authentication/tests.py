...
from rest_framework_security.authentication.middleware import (
...
class LoginAPIViewTestCase(APITestCase):
    ...
    def test_valid_login(self):
        response = self.client.post(
            self.url, {"username": "demo", "password": "demo1234"}, format="json"
        )
...
class AuthenticationMiddlewareTestCase(TestCase):
    ...
    @patch("rest_framework_security.authentication.middleware.get_next_steps")
    @patch(
        "rest_framework_security.authentication.middleware.is_path_allowed",
        return_value=True,
    )
    ...
    def test_validate_and_renew_session_logout(self, m):
        ...
        authentication_middleware.validate_and_renew_session(
            request_mock, {"ip_address": "old_ip"}
        )
        m.assert_called_once()
    ...
    @patch(
        "rest_framework_security.authentication.middleware.get_client_ip",
    ...
    def test_validate_and_renew_session_expired(self, m):
        ...
        self.assertGreater(
            UserSession.objects.get(pk=user_session.pk).session_expires, dt
        )
...
class GetAdminBaseUrlTestCase(TestCase):
    @patch(
        "rest_framework_security.authentication.next_steps.reverse",
        return_value="/admin/",
    )

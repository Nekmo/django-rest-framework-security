...
class GeoIPSerializerMixin(serializers.Serializer):
    ...
    def get_city(self, obj):
        ...
        return obj.geoip2_city_record.city.names.get(language_code, obj.geoip2_city_record.city.name)

    def get_country(self, obj):
        if obj.geoip2_city_record is None:
            return None
        language_code = self.context['request'].LANGUAGE_CODE
        return obj.geoip2_city_record.country.names.get(language_code, obj.geoip2_city_record.country.name)

    def get_continent(self, obj):
        if obj.geoip2_city_record is None:
            return None
        language_code = self.context['request'].LANGUAGE_CODE
        return obj.geoip2_city_record.continent.names.get(language_code, obj.geoip2_city_record.continent.name)

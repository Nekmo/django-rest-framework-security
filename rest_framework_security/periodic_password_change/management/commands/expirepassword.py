...
from rest_framework_security.periodic_password_change.models import PeriodicPasswordChangeSettingsBase
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('user', type=str,
                            help=_('Username, email or id.'))

    def handle(self, *args, **options):
        if not config.PERIODIC_PASSWORD_CHANGE_MODEL:
            raise CommandError('PERIODIC_PASSWORD_CHANGE_MODEL is undefined. To expire a user\'s '
        ...
        instance: PeriodicPasswordChangeSettingsBase = model_class.objects.get(user=user)
